"""
Waymo Unified JSON → training CSV.

Mirrors generate_csv_nuscenes.py and generate_csv_kitti.py exactly.
Run extract_waymo.py first to produce unified_waymo.json.
...
def calculate_fastest_vel(objects: list) -> float:
    ...
    max_v = 0.0
    for obj in objects:
        v_vec = obj.get("velocity", [0.0, 0.0])
...
def main():
    ...
    with open("unified_waymo.json", "r") as f:
        data = json.load(f)
    ...
    for scene in data:
        ...
        # Waymo is 10 Hz — dt matches KITTI
    ...
    output_file = output_dir / "waymo_training_data.csv"
