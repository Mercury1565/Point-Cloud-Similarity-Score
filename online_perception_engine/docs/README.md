# Online Perception Engine

The `online_perception_engine` is a dynamic, self-adapting system designed to optimize vehicle perception pipelines. It balances computational efficiency with perception reliability by using online learning to predict when previous perception results can be safely reused.
...
```bash
python -m online_perception_engine --csv-path data/training_data.csv --output-dir output
```
